    ...
    <?= DetailView::widget([
        ...
        'attributes' => [
            ...
            [
                ...
                'value' => function ($data) {
                    return $data->path ?
                        '<a href="//'.Yii::$app->urlManager->hostInfo.'/'.$data->path.'" target="_blank">'.Yii::t('base_mod', 'OPEN_MATERIAL').'</a>' :
                        '<a href="//'.Yii::$app->urlManager->hostInfo.'/article/'.$data->id.'" target="_blank">'.Yii::t('art_mod', 'OPEN_MATERIAL').'</a>';
                }
            ],
            [
                'attribute' => 'cat_id',
                ...
                'value' => function ($data) {
                    if ($data->cat_id === 0) {
                        $category = "<span class='has-error'>" . Yii::t('base_mod', 'PARENT_ERROR') . "</span>";
                    } else {
                        $findCategory = KnowledgeBaseCategory::findOne(['id' => $data->cat_id]);
                        if ($findCategory) {
